...
export default function Cart() {
  ...
  return (
    <motion.div
      ...
      className="fixed h-screen w-full left-0 top-0 bg-black/25"
    >
      {/* Cart */}
      <motion.div
        layout
        onClick={(e) => e.stopPropagation()}
        className="bg-white absolute right-0 top-0 w-full lg:w-2/5 h-screen p-12 overflow-scroll text-gray-700"
      >
        <button
              ...
              <motion.div layout key={item.id} className="flex py-4 gap-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => cartStore.reduceQuantity({
                      id: item.id,
                      image: item.image,
                      name: item.name,
                  ...
                  <div className="flex gap-2">
                    <h3>Remove</h3>
                    <button onClick={() => cartStore.deleteItem({
                      id: item.id,
                      image: item.image,
                      name: item.name,
                      unit_amount: item.unit_amount,
